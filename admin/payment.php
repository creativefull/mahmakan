<?php  
   ...
   $ShopidUs=$_SESSION['ShopidUs'];
 }
   $Tanggal=date("Y/m/d");  
   if (isset($_SESSION['items'])) {
        $sql='INSERT INTO charthd(transnmbr,transdate,customerid,statusid ) 
        select CONCAT('.'"'.$ShopidUs.'"'.',CONCAT(REPLACE(DATE(SYSDATE()),"-",""),REPLACE(TIME(SYSDATE()),":",""))),
         SYSDATE(),'.$ShopidUs.','."1";
   ...
   $sql='select max(id) nilai from charthd where customerid ='.$ShopidUs;
...
)  select (select max(id) from charthd where customerid ='.$ShopidUs.'),'.$ProductId.','.$val.','.$Price.','.$Price * $val.",0,".$Price * $val;
           //echo $sql;  
